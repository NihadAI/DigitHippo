...
export default async function Page ({params}: PageProps) {
  ...
  return (
    <MaxWidthWrapper className="bg-white">
                ...
                <div className="lg:max-w-lg lg:self-end">
                    <ol className="flex items-center space-x-2">
                        {BREADCRUMBS.map((breadcrumb, i) => (
                            <li key={breadcrumb.href}>
                                <div className="flex items-center texxt-small">
                                    <Link href={breadcrumb.href} className="font-md text-muted-foreground text-sm text-gray-900">{breadcrumb.name}</Link>
                    ...
                    <section className="mt-4">
                        <div className="flex items-center">
                            ...
                            <div className="border-l ml-4 text-muted-foreground bborder-gray-300 pl-4">{label}</div>
                        </div> 
        ...
        <ProductReel href="/products" query={{category: product.category, limit:4}} title={`Similar ${label}`} subtitle={`Browser similar products just like ${product.name}`}/>
    </MaxWidthWrapper>
  )
}
